...
let isDemoMode = typeof window !== 'undefined' ? localStorage.getItem('vglobal_shadow_mode') === 'true' : false;
...
export const PROXIES = {
  ...
  CODETABS: 'https://api.codetabs.com/v1/proxy?quest=https://www.saltedge.com/api/v5',
...
async function localApiRequest(endpoint: string, options: any = {}) {
  ...
  if (activeProxy === 'INTERNAL') {
    ...
    const baseUrl = PROXIES[activeProxy as keyof typeof PROXIES] || PROXIES.INTERNAL;
  ...
  try {
    ...
    try {
      ...
      console.error("Malformed API Response Received:", rawText.substring(0, 200));
      
      // If we got HTML (starts with <), it's likely a proxy error or server 404
      if (rawText.trim().startsWith('<')) {
        throw new Error(`SERVER_ERROR: Received HTML instead of JSON. The proxy or route may be misconfigured.`);
      }
      
      throw new Error(`PARSE_ERROR: Response was not valid JSON. Position 0: ${rawText.substring(0, 10)}...`);
    }

    if (!response.ok) {
      const errorMsg = result.error || result.error_message || result.message || `API Error ${response.status}`;
    ...
    if (error.message.includes('Failed to fetch') || error.message.includes('404') || error.message.includes('SERVER_ERROR')) {
...
function getMockData(endpoint: string) {
  if (endpoint.includes('/customers')) {
    return { data: [{ id: 'demo_user_v5', identifier: 'demo@verifyglobal.com', created_at: new Date().toISOString() }] };
  }
  if (endpoint.includes('/accounts')) {
    return { data: [
      { id: 'acc_demo_1', name: 'Demo Checking', balance: 5000.00, currency_code: 'USD', nature: 'checking' }
    ]};
  }
  return { data: [] };
}
...
export const saltedgeService = {
  ...
  createCustomer: (identifier: string) => localApiRequest('/customers', {
    method: 'POST',
    body: JSON.stringify({ identifier })
  }),
  createConnectSession: (customerId: string) => localApiRequest('/connect', {
    method: 'POST',
    body: JSON.stringify({ customer_id: customerId, return_to: window.location.origin })
  }),
  getAccounts: (customerId?: string) => localApiRequest(`/accounts${customerId ? `?customer_id=${customerId}` : ''}`),
  getTransactions: (connectionId?: string) => localApiRequest(`/transactions${connectionId ? `?connection_id=${connectionId}` : ''}`),

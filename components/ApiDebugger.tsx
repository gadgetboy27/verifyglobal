...
const ApiDebugger: React.FC = () => {
  ...
  const captureApiCall = (endpoint: string, method: 'GET' | 'POST' = 'GET', request?: any) => {
    const call: ApiCall = {
  ...
  const updateApiCall = (callId: string, response: any, duration: number, status: 'success' | 'error', error?: string) => {
    setCalls(prev =>
      prev.map(call =>
        call.id === callId
  ...
  const testEndpoint = async (endpoint: string, method: 'GET' | 'POST' = 'GET', data?: any) => {
    ...
    try {
      ...
      if (endpoint === '/customers' && method === 'POST') {
        response = await saltedgeService.createCustomer(data?.identifier || `user_${Date.now()}@test.com`);
  ...
  return (
    <div className="w-full max-w-6xl mx-auto bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
      <div className="mb-8">
        <h2 className="text-2xl font-black text-slate-900 mb-2">API Call Debugger</h2>
        <p className="text-slate-600">Monitor all API requests and responses in real-time</p>
      </div>
      ...
      <div className="bg-slate-50 rounded-lg p-6 mb-8 space-y-4">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="autoCapture" className="text-sm font-semibold text-slate-700">
        ...
        <div className="flex gap-2 border-t border-slate-200 pt-4">
          {['all', 'success', 'error'].map(tab => (
            ...
            >
              {tab.charAt(0).toUpperCase() + tab.slice(1)} ({calls.filter(c => (tab === 'all' ? true : c.status === tab)).length})
            </button>
          ))}
        </div>
      </div>
      ...
      {/* Quick Test Buttons */}
      <div className="mb-8">
        <h3 className="text-sm font-black text-slate-600 uppercase tracking-widest mb-4">Quick Tests</h3>
      ...
      <div className="space-y-2 mb-8">
        <h3 className="text-sm font-black text-slate-600 uppercase tracking-widest">API Calls ({filteredCalls.length})</h3>
        <div className="max-h-96 overflow-y-auto space-y-2 bg-slate-50 rounded-lg p-4">
          {filteredCalls.length === 0 ? (
            <p className="text-center text-slate-500 py-8">No API calls yet. Click a quick test button or make an API call.</p>
          ) : (
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <div
                      className={`w-2 h-2 rounded-full flex-shrink-0 ${
                        call.status === 'success' ? 'bg-emerald-500' : call.status === 'error' ? 'bg-rose-500' : 'bg-amber-500'
                      }`}
                    />
                    <span className="text-xs font-bold bg-slate-900 text-white px-2 py-1 rounded text-slate-100">
                      {call.method}
                    </span>
                    <span className="text-sm font-bold text-slate-900 truncate">{call.endpoint}</span>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="text-xs font-bold text-slate-600">{call.duration}ms</p>
                    <p className="text-[10px] text-slate-500">{call.timestamp.toLocaleTimeString()}</p>
                  </div>
                </div>
              </button>
            ))
      ...
      {selectedCall && (
        <div className="border-t-2 border-slate-200 pt-8">
          <h3 className="text-sm font-black text-slate-600 uppercase tracking-widest mb-4">Call Details</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-slate-50 p-4 rounded-lg">
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Endpoint</p>
              <p className="font-mono text-sm text-slate-900">{selectedCall.method} {selectedCall.endpoint}</p>
            </div>
            <div className="bg-slate-50 p-4 rounded-lg">
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Status</p>
              <p
                className={`font-bold text-sm capitalize ${
                  selectedCall.status === 'success' ? 'text-emerald-600' : selectedCall.status === 'error' ? 'text-rose-600' : 'text-amber-600'
                }`}
              >
                {selectedCall.status}
              </p>
            ...
            <div className="bg-slate-50 p-4 rounded-lg">
              ...
              <p className="font-mono text-sm text-slate-900">{selectedCall.duration}ms</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {selectedCall.request && (
              <div>
                <h4 className="text-xs font-bold text-slate-600 uppercase tracking-widest mb-2">Request</h4>
            ...
            {selectedCall.response && (
              <div>
                <h4 className="text-xs font-bold text-slate-600 uppercase tracking-widest mb-2">Response</h4>
            ...
            {selectedCall.error && (
              <div className="lg:col-span-2">
                <h4 className="text-xs font-bold text-slate-600 uppercase tracking-widest mb-2">Error</h4>
